...
def test_unmatched_frame_reports_best_score(tmp_path):
    template = np.zeros((5, 5), np.uint8)
    template[1, 2] = 255
    cv2.imwrite(str(tmp_path / "monster.png"), template)
    detector = TemplateDetector(tmp_path, 0.999, 10)
    detector.load()
    result = detector.detect(np.zeros((30, 30, 3), np.uint8))
    assert result.reliable
    assert result.detections == []
    assert "monster.png" in result.template_scores
